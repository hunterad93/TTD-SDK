...
class CreativeResource:
    ...
    def list_recently_created(
        ...
        """
        Get a paginated list of recently created creatives for an advertiser.
        
        Args:
            advertiser_id: The platform ID of the advertiser
            days_back: Number of days back to look for recently created creatives (default: 7)
            page_size: Number of results per page (100-1000 recommended)
            search_terms: Optional list of search terms to filter by
            availabilities: Optional list of availability states to filter by
        """
        ...
        for result in self.client.post_with_pagination(
            ...
            creative = ApiObject(**result)
            # Filter by creation date - stop when we reach older creatives
            if hasattr(creative, 'CreatedAtUTC') and creative.CreatedAtUTC < cutoff_date_str:
                break
            yield creative